<template>
	<div class="all">
		...
		<div id="middle">
			...
			<div class="btn_list">
				<button  type="button" class="ban" v-if="info.r_status == 0" @click="caozuo(1)">同意预约</button>
				<button  type="button" class="ban" v-if="info.r_status == 0" @click="caozuo(2)">拒绝预约</button>
				<button  type="button" v-if="info.r_status == 3">租客已取消预约</button>
			</div>
		</div>
	...
	export default {
		...
		methods:{
			get_detail(id){
				...
				this.$axios({
					url:'http://wy.gzziyu.com/zlisting.php?Action=yezhuyuyuedetail',
					method:'post',
					params:{
						r_id:id,
			...
			caozuo(index){
				this.$axios({
					url:'http://wy.gzziyu.com/zlisting.php?Action=yezhuquxiaoyuyue',
					method:'post',
					params:{
						r_id:this.id,
						user_id:userInfo.user_id,
...
<style lang="scss" scoped>
	@import './bespeak_house_detail.scss'
</style>